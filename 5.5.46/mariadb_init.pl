...
die "$0: missing: $cwd/share/english/errmsg.sys" if
    !-f "$cwd/share/english/errmsg.sys";
my $cwdq = $cwd;
...
sub get_defaults() {
  ...
  if (defined $defaults->{language}) {
    if ($defaults->{language} !~ m@/@) {
      ...
      $defaults->{language} = "./share/$defaults->{language}";
    ...
    $defaults->{'.language'} = './share/english';
  }
...
sub stop($) {
  ...
  if ($do_set_root_password) {
    ...
    die "$0: could not start password changing command: $!\n"
        ...
        "--datadir=$cwdq/data --language=$cwdq/share/english ".
        "--console --log-warnings=0 --loose-skip-pbxt " .
